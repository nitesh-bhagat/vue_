<template>
    <div class="flex flex-col gap-2">
        <div class="flex flex-row px-3 pt-3 gap-2 items-center">
            <!-- logo -->
            <h1 class="mr-auto text-4xl font-bold text-green-500"> Score<span class="text-black">pion</span></h1>
        ...
        <div class="flex flex-row w-full">
            <button v-for="(menu, index) in getTabList" :key="index"
                :class="[getCurrentTab === index && 'text-green-500  border-green-500', 'border-b-2 py-2 px-4']"
                @click="switchTab(index)">{{ menu.title }}</button>
